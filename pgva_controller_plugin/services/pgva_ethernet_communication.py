...
class PGVAEthernetCommunication:
    ...
    def __init__(self, ip_address: str = "192.168.0.1", port: int = 502,
                 unit_id: int = 0, timeout: float = 5.0):
        """
        ...
        self.client = ModbusTcpClient(host=ip_address, port=port, timeout=timeout)
    ...
    def perform_aspirate_operation(self, pressure_mbar: float, time_ms: int,
        ...
        Args:
            ...
            cancel_event: optional threading.Event; if set while waiting for
                pressure, the wait is aborted early and this method returns False
        Returns:
            True if successful, False otherwise
        """
        if not self.connected:
        ...
        try:
            ...
            if not self._wait_for_target_pressure(cancel_event=cancel_event):
                logger.error("Timeout (or cancel) waiting for target pressure")
                return False
    ...
    def perform_dispense_operation(self, pressure_mbar: float, time_ms: int,
                                    cancel_event=None) -> bool:
        """
    ...
    def _wait_for_target_pressure(self, timeout_seconds: int = 30, cancel_event=None) -> bool:
        """
        Wait for the output pressure to reach the target value.

        Args:
            timeout_seconds: maximum time to wait
            cancel_event: optional threading.Event; if set, the wait aborts
                early (used to unblock this loop on application shutdown)
        """
        ...
        while ((elapsed < timeout_seconds) and
            ...
            if cancel_event is not None and cancel_event.is_set():
                logger.info("Pressure wait cancelled")
                return False
