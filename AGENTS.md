...
- **Decoupling**: plugins never call each other directly — communicate via
  ...
  `pixi run lint-imports` (or `pixi exec --spec "import-linter" --
...
- Link the PR to its issue with a **closing keyword** (`Closes #123`, `Fixes
  #123`) rather than a bare mention — the closing reference is what moves the
  issue on the project board and closes it on merge; a prose mention leaves
  both stale
