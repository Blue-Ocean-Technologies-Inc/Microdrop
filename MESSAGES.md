...
- Live stream & acquire's **Move to** dropdown (Park = spot 0, then the board's configured spots) publishes `PMT_MOVE_TO_SPOT = "portable_dropbot/requests/pmt_move_to_spot"` (the spot number as text) as soon as it changes; the backend moves via `motor.pmt_ctrl(spot + PMT_PARK_LOCATION)`, refusing (logged) while a PMT capture runs — a live stream or acquire keeps running. **Home PMT** publishes the Motors pane's `MOTOR_HOME` with `"pmt"` and resets the dropdown to Park without a second move.
...
**Capture routine**
- Start publishes `PMT_CAPTURE = "portable_dropbot/requests/pmt_capture"` — `PmtCaptureRequest {entries: [{slot, gain, exposure_s}], avg, osr, rf_ohms, request_id}` (the stream settings inherited from `PmtStreamSettings`, defaulting to `PMT_STREAM_AVG`/`PMT_STREAM_OSR`/`PMT_RF_OHMS`), ticked rows in table order. The pane mints a fresh `uuid4()` per Start and stores it as `capture_request_id`, the capture it is now following (see "Pane follows only its own capture" below).
- The backend runs the routine on a daemon thread (the actor's time limit would otherwise interrupt a long capture): ADC query, fluorescence LED off, PMT power on, then per entry move (`MotorBoardProxy.pmt_ctrl`), gain, `SignalBoardProxy.pmt_stream(1, request.avg, request.osr)` for `exposure_s` while a `uart.subscribe(CMD_PMT_STREAM_DATA)` callback assembles the frames, stream stop, CSV to `<experiment>/captures/pmt/pmt_spot<slot>_<stamp>_gain<g>.csv` (the queried `adc_full_scale`, `request.avg`/`osr`/`rf_ohms` in its meta). Each stage publishes `PMT_CAPTURE_PROGRESS = "portable_dropbot/signals/pmt_capture_progress"` (`PmtCaptureProgress {index, total, slot, stage, detail, exposure_s, request_id}`; stages move/gain/stream/saved/failed; `exposure_s` is set on the stream stage so the pane counts the exposure down on its status line and highlights that spot's row; `request_id` echoes the request's, so the pane can tell its own capture's progress from a protocol step's). The proxy lock is taken per command so `STATUS_UPDATED` keeps flowing during a long exposure.
...
**Pane follows only its own capture (#702)** — the PMT Capture and Fluorescence Capture panes share `CapturePaneModel.capture_request_id`: Start mints a fresh `uuid4()`, stores it there, and hands it to the request; `_on_..._capture_progress_triggered`/`_on_..._capture_done_triggered` compare the message's `request_id` against it (`CapturePaneMessageHandler._is_own_capture`) and do nothing for a mismatch — so a protocol step's capture (`request_id=f"{row.uuid}:{phase}"`, see below) never drives the pane's status line, active-row highlight, Results table or (PMT) exposure countdown. A matching `..._DONE` clears `capture_request_id`, including a refusal ack (`directory=""`) for the pane's own request, which still ends `capturing` and shows the error as before.
...
**Protocol-step captures (#601 increment 2, `portable_dropbot_protocol_controls`'s `pmt_capture` column)** — `PmtCaptureRequest` carries three more optional fields, all defaulted so a pane-initiated request is unaffected:
  - `request_id` (default `""`, but always set by both the pane and a protocol step now — see "Pane follows only its own capture" above) is echoed back verbatim on `PmtCaptureDone`, on success AND on every refusal (`_refuse_pmt_capture` takes it as a parameter; a request that fails Pydantic validation still gets it read best-effort off the raw JSON, so even a malformed request's ack can be matched, or `""` if the payload wasn't even readable as JSON). A protocol step's `PmtCaptureHandler` sets it to `f"{row.uuid}:{phase}"` (`phase` is `start` or `end`) and waits on `PMT_CAPTURE_DONE` with a `predicate=` matching that value, so a pane-initiated capture or a stale done from an earlier step can never satisfy its wait — the same pattern the droplet-check column uses with `step_uuid`.
...
**Pane (`portable_dropbot_status_and_controls`, "Fluorescence Capture" dock pane)** — one row per `FILTER_POSITIONS` entry (capture tick, LED %, auto exposure, exposure ms, start/end ticks; no focus control — the Pi's camera has no software focus, so entries always carry `focus_distance: null`). Start publishes `FLUORESCENCE_CAPTURE` with `request_id=uuid4()` (also stored as `capture_request_id`, the capture the pane now follows — see the PMT section's "Pane follows only its own capture" for the shared mechanism), `label="manual"`; Abort publishes `FLUORESCENCE_CAPTURE_ABORT`. Attaching to the selected protocol row writes the step's cell over `PROTOCOL_TREE_SET_CELL`, mirroring the PMT Capture pane.

**Filter names** — positions stay numbers in every message and step cell; `FILTER_NAMES` (1 HEX, 2 CY5, 3 White, 4 FAM, 5 Empty — confirmed against the vendor UI) is display-only: the pane shows `filter_label()` (the dye alone, "FAM") in its tables, Filter pick and status line, and `frame_description()` puts the dye in each frame's `step_description`, so files read `flu_<label>_f4_FAM_<timestamp>.png`.

**Park PMT / Park Filter (the capture panes)** — a pane-level checkbox (off by default) beside Abort (fluorescence) / Refresh spots (PMT). It rides on `PMT_CAPTURE` / `FLUORESCENCE_CAPTURE` as `park_motor` (`CaptureParkOption`), and on an attached step's `pmt_capture` / `fluorescence_capture` cell the same way (the protocol columns pass `step.park_motor` through, adding one per-entry overhead to the step timeout). When set, the capture's guaranteed teardown also moves its motor to its parking position — the PMT to park (spot 0, `PMT_PARK_LOCATION`), the filter wheel to `FLUORESCENCE_PARK_FILTER` (3) — whether the run finished, failed or was aborted (best effort, logged on failure).

**PMT / fluorescence exclusivity (per step)** — a step sets only one of `pmt_capture` / `fluorescence_capture`. Each capture pane reads the other column from the `PROTOCOL_TREE_ROW_SELECTED` cells: while the selected step carries the other capture and its own cell is empty, its Start/End table is locked with a note (a step carrying both stays editable so one can be cleared). Since a set-cell rebroadcasts `PROTOCOL_TREE_ROW_SELECTED`, ticking in one pane locks the other live. At run time both columns call `check_single_capture` (`portable_dropbot_protocol_controls/capture_exclusivity.py`) at the start of each phase, so a step with both raises before any hardware moves.

**Pane Manual controls (same pane, collapsible group)** — each edit applies live (the LED is left to the status pane's Light control): a filter pick publishes `SET_FILTER` (the position as text); **Home filter** publishes `MOTOR_HOME` with `"filter"` and resets the pick to position 1 without a second move; an exposure change (while not on auto) or ticking Auto exposure publishes a `CameraControlsRequest` on `DEVICE_VIEWER_CAMERA_SET_CONTROLS` with `request_id="manual"` (`exposure_ms` `None` = auto; `focus_distance` always `None`); unticking it publishes `hold_auto_exposure=true` with `request_id="manual-hold-exposure"` only when the last readback reported auto's pick (that readback's `exposure_ms` then moves the Exposure slider there), else a plain manual request at the slider's value. Capture frame publishes `DEVICE_VIEWER_SCREEN_CAPTURE` `{directory: <experiment dir>, step_description: "flu_manual_f<n>", show_dialog: false, request_id: "manual-<uuid4>"}`. The pane's listener subscribes to `DEVICE_VIEWER_CAMERA_CONTROLS_APPLIED` (any readback, manual or a capture's, shown on its Camera line) and `DEVICE_VIEWER_MEDIA_CAPTURED` (only the file echoing its in-flight `manual-` request id becomes a Results run).
