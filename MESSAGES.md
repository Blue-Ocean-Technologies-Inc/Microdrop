...
**Filter names** — positions stay numbers in every message and step cell; `FILTER_NAMES` (1 HEX, 2 CY5, 3 White, 4 FAM, 5 Empty — confirmed against the vendor UI) is display-only: the pane shows `filter_label()` ("4 · FAM") in its tables, Filter pick and status line, and `frame_description()` puts the dye in each frame's `step_description`, so files read `flu_<label>_f4_FAM_<timestamp>.png`.
