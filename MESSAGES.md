...
- `FLUORESCENCE_CAPTURE_DONE = "portable_dropbot/signals/fluorescence_capture_done"` — `FluorescenceCaptureDone {request_id, ok, label, directory, paths, error}`; `paths` are the saved PNGs, capture order.
...
- Teardown always runs two independent steps: restore the LED to the controller's current light intensity, and reset the camera to auto exposure/focus (fire-and-forget, no wait). Then `FLUORESCENCE_CAPTURE_DONE` with the paths collected so far — `ok=False` and `error` set on a filter alarm, a camera refusal, a wait timeout, or an abort (`error="aborted"`).
