...
- `DEVICE_VIEWER_CAMERA_CONTROLS_APPLIED = "ui/device_viewer/camera/controls_applied"` — `CameraControlsApplied {request_id, ok, exposure_ms, exposure_auto, focus_distance, error}` (`exposure_ms` is the exposure in effect — auto's current pick while `exposure_auto`, read from v4l2 `exposure_time_absolute` on the Pi — or `None` when unreported), the camera's readback after applying; a provider feed (no `QCamera`) or an unsupported mode answers `ok=False` with why, rather than raising.
