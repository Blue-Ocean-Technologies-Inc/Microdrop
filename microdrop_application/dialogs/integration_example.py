...
class MicrodropDialogIntegration:
    ...
    def setup_application_logger(self):
        ...
        # Add dialog handler
        dialog_handler = add_dialog_handler_to_logger(logger.name, show_dialogs=self.dialogs_enabled)

        # Configure dialog behavior for different levels
        dialog_handler.update_config(
            "CRITICAL",
            DialogConfig(
    ...
    def show_save_success(self, parent=None, filename=""):
        ...
        return show_success(parent=parent, title="Save Complete", message=message)
    ...
    def show_protocol_warning(self, parent=None):
        ...
        return show_question(
            ...
            message="You have unsaved changes to the current protocol. " "Save before continuing?",
            yes_text="Save",
            no_text="Discard",
        )
    ...
    def show_device_info(self, parent=None, device_info=None):
        ...
        return show_information(parent=parent, title="Device Information", message=message)
...
def show_microdrop_error(parent=None, title="Error", message="", error_details=None):
    """Show a Microdrop-styled error dialog."""
    return show_error_alert(parent=parent, title=title, message=message, error_details=error_details)


def show_microdrop_success(parent=None, title="Success", message=""):
    """Show a Microdrop-styled success dialog."""
    return show_success(parent=parent, title=title, message=message)


def show_microdrop_info(parent=None, title="Information", message=""):
    """Show a Microdrop-styled information dialog."""
    return show_information(parent=parent, title=title, message=message)


def show_microdrop_question(parent=None, title="Question", message="", yes_text="Yes", no_text="No"):
    """Show a Microdrop-styled question dialog."""
    return show_question(parent=parent, title=title, message=message, yes_text=yes_text, no_text=no_text)
...
def example_usage():
    ...
    integration.show_device_info(device_info={"name": "DropBot v3.1", "version": "3.1.2", "status": "Connected"})
