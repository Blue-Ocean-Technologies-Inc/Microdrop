...
class LoggerDialogHandler(logging.Handler, QObject):
    ...
    def _should_show_dialog(self, record: logging.LogRecord) -> bool:
        ...
        if hasattr(record, "msg"):
            ...
            if any(keyword in msg for keyword in ["critical", "fatal", "emergency", "panic"]):
                return True

            # Show dialogs for user-facing errors
            if any(keyword in msg for keyword in ["user error", "input error", "validation error"]):
                return True

        # Default behavior for WARNING and above
        return record.levelno >= logging.WARNING

    # Mapping of dialog types to classes and parameters
    _DIALOG_TYPE_MAP = {
        BaseMessageDialog.TYPE_ERROR: {
            "class": ErrorAlertDialog,
            "extra_args": ["error_details"],
        },
        BaseMessageDialog.TYPE_SUCCESS: {"class": SuccessDialog},
        BaseMessageDialog.TYPE_WARNING: {
            "class": WarningAlertDialog,
            "kwargs": {"exit": True},
        },
        BaseMessageDialog.TYPE_INFO: {"class": InformationDialog},
    }

    def _display_dialog_from_signal(self, level: str, message: str, logger_name: str, config: Dict[str, Any]):
        """
        ...
        try:
            ...
            type_config = self._DIALOG_TYPE_MAP.get(dialog_type, self._DIALOG_TYPE_MAP[BaseMessageDialog.TYPE_INFO])
            dialog_class = type_config["class"]
            kwargs = type_config.get("kwargs", {}).copy()

            # Extract specific arguments
            for arg_name in type_config.get("extra_args", []):
                if arg_name in config:
                    kwargs[arg_name] = config[arg_name]

            # Instantiate dialog
            dialog = dialog_class(parent=parent, title=title, message=message, **kwargs)
            ...
            dialog.dialog_closed.connect(lambda: self._cleanup_dialog(dialog_key))
