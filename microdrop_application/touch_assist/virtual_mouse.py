...
class VirtualMouse(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._press_zone == "wheel":
            ...
            return
        if delta.manhattanLength() > MOUSE_TAP_SLOP_PX or self._press_zone == "body":
    ...
    def mouseReleaseEvent(self, event):
        ...
        if pressed is None or zone in ("wheel", "body"):
            return
        travelled = (event.globalPosition().toPoint() - pressed).manhattanLength()
        ...
        if zone == "left" and self._held_target is not None:
            ...
            return
        self._send_click(
            Qt.MouseButton.LeftButton if zone == "left" else Qt.MouseButton.RightButton,
            zone,
        )
    ...
    def _propagate(self, target, event_factory):
        ...
        while target is not None:
            event = event_factory(QPointF(target.mapFromGlobal(global_pos)))
    ...
    def _send_click(self, button, zone):
        ...
        double = (
            zone == last_zone
            and (now - last_time) * 1000 <= QApplication.doubleClickInterval()
        )
        ...
        press = (
            QEvent.Type.MouseButtonDblClick if double else QEvent.Type.MouseButtonPress
        )
        ...
        if button == Qt.MouseButton.RightButton:
            ...
            self._propagate(
                target,
                lambda local_pos: QContextMenuEvent(
    ...
    def _send_wheel(self, notches):
        ...
        if self._wheel_target is None or not shiboken6.isValid(self._wheel_target):
            resolved = self._target()
            if resolved is None:
                return
    ...
    def paintEvent(self, event):
        ...
        painter.drawLine(
            wheel_left + MOUSE_WHEEL_WIDTH_PX,
            body.top(),
            wheel_left + MOUSE_WHEEL_WIDTH_PX,
            split_y,
        )
        ...
        right_zone = QRect(
            wheel_left + MOUSE_WHEEL_WIDTH_PX,
            body.top(),
            body.right() - wheel_left - MOUSE_WHEEL_WIDTH_PX,
            split_y,
        )
        ...
        painter.setBrush(QColor(PRIMARY_COLOR) if held else QColor(GREY["lighter"]))
        ...
        painter.drawText(
            QRect(body.left(), split_y, body.width(), body.bottom() - split_y),
            Qt.AlignCenter,
            "drag\nto aim",
        )
