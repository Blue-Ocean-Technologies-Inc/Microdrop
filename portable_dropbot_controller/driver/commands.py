...
class MotorBoard:
    ...
    MOTOR_HOMED_QUERY = (HEAD << 8) | 0xB5  # Query per-motor homed status, returns u8[6]
    ...
    PARAMS = {
        ...
        # [2026-08-01] Re-enabled: present + populated on the new motor board
        # (factory DroDri_1.0.1.16 EF dump) and registered by our firmware —
        # excluding them left real settings out of flash-time backup/restore.
        "temp_defaults": "_dp_temp",
        "target_positions": "_dp_tpos",
        "pmt_home_dir": "_dp_ydir",  # [2026-08-01] Y home-search direction (1=far side/original, 0=near side/new mechanics)
...
class SignalBoard:
    ...
    POWER_CTRL = (HEAD << 8) | 0xA4  # Power: u8 1=on, 0=off (button-equivalent since ba54150: on-from-OFF writes BKP flag + resets into the real power-on path; ACK arrives BEFORE the reset)
