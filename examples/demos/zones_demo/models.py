...
"""Qt-free traits models for the electrode zones demo.

Traits sketch from docs/superpowers/specs/2026-08-26-electrode-zones-design.md:
...
class ZoneType(HasTraits):
    ...
    #: so the table cell still opens the standard color picker while the model
    #: stays a serializable string.
    color = Str()

    #: Number of regions of this type; maintained by ZoneLayerManager.
    region_count = Int(0)

    #: Fired by the zone-types table's per-row delete glyph; the controller
    #: removes the type (and, via the manager, all its regions).
    delete_requested = Event()


class ZoneRegion(HasTraits):
    #: Stable key within the manager, e.g. "heating-1"; assigned on creation.
    id = Str()
...
class ZoneLayerManager(HasTraits):
    ...
    #: merging), a plain click collapses back to one. ``selected_region``
    #: stays the primary (last-clicked) member.
    ...
    #: zone id -> highest region number ever handed out; never reused, so a
    #: deleted region's id can't silently rebind to a new region.
    _region_id_counters = Dict(Str, Int)

    #: region -> (electrode-id tuple, computed outline geometry). Selection
    #: restyles trigger full redraws; this keeps them from recomputing
    #: every region's shapely union each time.
    _outline_cache = Dict()

    #: Undo snapshots (plain dicts), newest last; capped at UNDO_STACK_LIMIT.
    _undo_stack = List()
    ...
    def add_zone_type(self, name, color):
        self._push_undo()
        zone_type = ZoneType(id=self._generate_zone_id(name), name=name, color=color)
    ...
    def remove_from_pending(self, electrode_ids):
        ...
        ctrl+rubber-band gesture in draw/edit mode)."""
    ...
    def toggle_region_in_selection(self, region):
        ...
        ``selected_region`` is derived from the list (its last element), so
        it lands on the toggled region when adding and the last remaining
        one when removing."""
        if region is None:
            return
    ...
    def merge_selected_regions(self):
        ...
        zone. Applies only when at least two regions are selected, they all
        belong to the same zone, and the union of their electrodes is one
        contiguous blob (overlapping or bordering regions) — otherwise
    ...
    def _translated_electrode_ids(self, region, dx, dy):
        ...
        plus the delta — or None when the move is invalid: a member maps off
        the known electrodes, two members collapse onto one target (shape
        not preserved), or the result is no longer one contiguous blob."""
        centroids = {
    ...
    def translate_region(self, region, dx, dy):
        """Move one region by (dx, dy); see _translated_electrode_ids for
        the validity rules. Returns True when applied."""
        return self.translate_regions([region], dx, dy)

    def translate_regions(self, regions, dx, dy):
        """Move several regions by the same snapped delta, all-or-nothing:
        every region must translate validly or none moves. (Regions may
        overlap by design, so no cross-region collision check.)"""
        moves = []
    ...
    def _connected_components(self, electrode_ids):
        ...
        walking the device's own neighbour graph (two electrodes are
        contiguous when the SVG says they are adjacent)."""
        remaining = {
            electrode_id
            for electrode_id in electrode_ids
    ...
    def capture_electrode_ids_touching(self, min_x, min_y, max_x, max_y):
        """Electrode ids whose polygons overlap the given rectangle (SVG
        coordinates) — the rubber-band capture rule: touching any part of an
        electrode is enough to select it."""
    ...
    def region_outline(self, region):
        ...
        outline hugs the block's outer corners (corner snapping for free).
        Cached per region keyed by its electrode set — selection restyles
        redraw every region, and only the pen changes then."""
    ...
    def _get_outline_closing_distance(self):
        ...
        if not math.isfinite(smallest_gap):
            return 0.0
        return OUTLINE_GAP_CLOSING_FRACTION * smallest_gap
    ...
    def _push_undo(self):
        self._undo_stack = (self._undo_stack + [self._snapshot()])[-UNDO_STACK_LIMIT:]
        # A new operation forks history; the redone future is gone.
        self._redo_stack = []

    def undo(self):
        """Restore the state before the last mutating operation (commit,
        delete, merge, move, type change). Selection, pending edits, and the
        in-progress edit are cleared — the restored objects are rebuilt."""
        if not self._undo_stack:
            return False
        state = self._undo_stack[-1]
        self._undo_stack = self._undo_stack[:-1]
        self._redo_stack = (self._redo_stack + [self._snapshot()])[-UNDO_STACK_LIMIT:]
        self._restore(state)
        return True
    ...
    def change_region_zone(self, region, zone_id):
        ...
        the new zone's namespace (ids embed the zone, and are never
        reused)."""
    ...
    def channels_for(self, zone_id):
        ...
        overlapping regions of one zone claim each electrode once, and an
        electrode may belong to several zones at once (overlap is allowed
        and blends visually); consumers resolve a zone to this union."""
        channels = set()
...
class ZonesDemoModel(HasTraits):
    """UI state for the demo window. The zone data itself lives on the
    manager; the sidebar-facing traits delegate to it so the demo View can
    bind everything on this one object."""

    manager = Instance(ZoneLayerManager, ())

    zone_types = DelegatesTo("manager")
    regions = DelegatesTo("manager")
    selected_zone_type = DelegatesTo("manager")
    selected_region = DelegatesTo("manager")
    selected_regions = DelegatesTo("manager")
    ...
    #: Canvas interaction mode, driven by the sidebar radio row.
    mode = Enum(PAN_MODE, ZONE_DRAW_MODE, SELECT_MODE, EDIT_MODE)

    #: Name typed for the next zone type added via ``add_zone_type_button``.
    new_zone_type_name = Str()

    #: Whether the floating button sets show on the canvas at all. The
    #: overlays' dismiss button turns this off; the sidebar checkbox brings
    #: them back (every overlay action also has a sidebar button).
    ...
    remove_zone_type_button = Button("Remove selected type")
