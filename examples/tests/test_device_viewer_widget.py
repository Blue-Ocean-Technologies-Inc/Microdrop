...
sample_svg_path_with_scale = (
    Path(TEST_PATH) / "device_svg_files" / "2x3device_with_scale.svg"
)
sample_svg_valid_channels_electrode_ids_map = (
    Path(TEST_PATH) / "valid_2x3_device_channels_electrode_ids_map.json"
)
...
def test_electrode_creation_traits_check_path_pass():
    ...
    assert test_electrode.channel == 1 and np.array_equal(
        test_electrode.path, correct_path_array
    )
...
def test_electrodes_creation_traits_check_fail():
    ...
    try:
        Electrodes(
            electrodes={
                "1": Electrode(channel=1, path=correct_path_array),
...
def test_get_channels_electrode_ids_map(valid_electrodes_model_from_svg):
    """
    Test method to get map of channels to electrode ids that have this
    channel associated with them. Some channels should have multiple
    electrode ids (like channel 30 for the example device svg).
    """
    ...
    for channel in valid_electrodes_model.keys():
        ...
        assert (
            valid_electrodes_model[channel]
            == valid_electrodes_model_from_svg.channels_electrode_ids_map[int(channel)]
        )
...
def test_save_svg_file_init_scale_metadata(valid_electrodes_model_from_svg):
    ...
    channels_to_svg(
        valid_electrodes_model_from_svg.svg_model.filename,
        new_filename,
...
def test_save_svg_file_edit_scale_metadata(valid_electrodes_model_from_svg_with_scale):
    ...
    channels_to_svg(
        valid_electrodes_model_from_svg_with_scale.svg_model.filename,
        new_filename,
