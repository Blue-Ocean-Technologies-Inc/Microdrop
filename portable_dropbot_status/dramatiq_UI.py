...
class DramatiqDropBotStatusViewModel(HasTraits):
    ...
    def _on_board_status_update_triggered(self, body):
        ...
        def get_formatted_measurement(key, unit):
            val = msg.get(key)
            # Only apply unit if the value is a valid number (int or float)
            if isinstance(val, (int, float)):
                return ureg.Quantity(val, unit)
            return "-"

        formatted_freq_measurement = get_formatted_measurement("hv_freq", ureg.hertz)
        if formatted_freq_measurement != "-":
            self.model.frequency = f"{formatted_freq_measurement.to_compact():.5g~P}"
        else:
