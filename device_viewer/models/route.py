...
class Route(HasTraits):
    ...
    # List of ids - ids can be anything! In this case they are most likely
    # strings or ints though
    route = List()
    ...
    def get_name(self, channel_map: dict[int, list]) -> str:
        ...
        if len(self.route) == 0:
            ...
            start_channel = self.get_channel_from_id(self.route[0], channel_map)
            if loop_count > 1:
                return f"{loop_count}x loop @ {start_channel}"
            else:
                return f"Loop @ {start_channel}"
        else:
            start_channel = self.get_channel_from_id(self.route[0], channel_map)
            end_channel = self.get_channel_from_id(self.route[-1], channel_map)
            return f"Path: {start_channel} -> {end_channel}"

    @staticmethod
    def is_segment(from_a, to_a, from_b, to_b) -> bool:
        """Returns if segment a is equivalent to segment b (equal or equal reversed)"""
        return (from_a == from_b and to_a == to_b) or (
    ...
    def can_merge(self, other: "Route") -> bool:
        ...
        return (
            self_endpoints[0] == other_endpoints[1]
            or self_endpoints[1] == other_endpoints[0]
        )

    def merge(self, other: "Route") -> list:
        """Merge with other route. Does this in place and does not modify
        the other route. Prioritizes putting other at end in ambigous
        cases. Assumes can_merge returns True"""
    ...
    def add_segment(self, from_id, to_id):
        ...
        # The order of these cases is *very* important! We prioritize
        # appending to end and in-direction, in that order!
        # Changing the order will add the segment in a technically valid
        # but strange place
        if len(self.route) == 0:  # Path is empty
            ...
            # We're extending from the middle! We actually add 2 segments:
            # the segment and its reverse, and use it to extend every part
            # of the path that has from_id
            new_route = []
    ...
    def remove_segment(self, from_id, to_id) -> list[list]:
        """Returns a list of new routes (in no particular order) that
        result from removing a segment from a given path (and merging
        pieces). Object should be dereferenced afterwards"""
        if len(self.route) == 0:
            return [[]]
        ...
        # Note that the order and count of channels do not change, so the
        # first and last element being equal still indicates a loop
        for i in range(len(self.route) - 1):
            new_routes[-1].append(self.route[i])
            if Route.is_segment(
                from_id, to_id, self.route[i], self.route[i + 1]
            ):  # Check both ways from -> to, to -> from
                # Terminate current segment and create new one
                new_routes.append([])
        new_routes[-1].append(self.route[-1])  # Add final element

        # Now we do the merge phase. We'll merge (concatonate) new_routes A
        ...
        while merge_flag:
            ...
            for i in range(len(new_routes)):
                ...
                for offset in range(
                    len(new_routes) - 1
                ):  # Look for closest possible merge
                    j = (i + 1 + offset) % len(new_routes)  # Add offest and loop around
                    if len(new_routes[j]) == 0:
                        continue
                    if (
                        new_routes[i][-1] == new_routes[j][0]
                    ):  # If condition 2 is met, merge
                        new_routes[i] = new_routes[i] + new_routes[j][1:]
                        new_routes[
                            j
                        ] = []  # We cant delete anything since it would break our loops
                        merge_flag = True
                        break

        return list(
            filter(lambda route: len(route) > 1, new_routes)
        )  # Remove empty/singular routes
...
class RouteLayer(HasTraits):
    ...
    # These traits are direct derivatives from a RouteLayerManager traits.
    # Do not modify from the Layer itself, only read
    is_selected = Bool(False)  # Needed to show selectedness in the TableEditor
    merge_in_progress = Bool(False)
    execution_disabled = Bool(
        False
    )  # True when protocol/step mode is running; disables Execute Path menu
...
class RouteLayerManager(HasTraits):
    ...
    repeats_frozen = Property(observe="linear_repeats,layers.items.route.route.items")
    ...
    def replace_layer(self, old_route_layer: RouteLayer, new_routes: list[Route]):
        ...
        layers_to_add = []
        # Add in new routes in the same place the old route was, so a new
        # route is preselected
        for i in range(len(new_routes)):
            # Maintain color of old route for the case of 1 returned,
            # visual persistence
            if i == 0:
                layers_to_add.append(
                    RouteLayer(route=new_routes[i], color=old_route_layer.color)
                )
            else:
                ...
                layers_to_add.append(
                    RouteLayer(
                        route=new_routes[i],
                        color=self.get_available_color(exclude=new_colors),
                    )
                )

        self.layers[index : index + 1] = reversed(
            layers_to_add
        )  # Delete and replace in single operation for easy undo
    ...
    def merge_layer(self, other_layer) -> bool:
        """Try to merge other_layer with layer_to_merge. Returns boolean
        indicating operation's success"""
        if self.layer_to_merge.route.can_merge(other_layer.route):
            ...
            # This should set layer_to_merge to None (for consistency) and
            # mode to something else
            index = self.replace_layer(self.layer_to_merge, [new_route])
    ...
    def _layers_items_changed(self, event):
        if (
            self.layer_to_merge is not None and self.layer_to_merge not in self.layers
        ):  # Clean up merge reference
            self.layer_to_merge = None
            self.mode = "edit"
    ...
    def _layer_to_merge_changed(self, event):
        if event.name == "layer_to_merge":  # event.new is the layer
            for layer in self.layers:
                layer.merge_in_progress = event.new is not None
            if event.new:
