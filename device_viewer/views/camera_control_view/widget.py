...
from PySide6.QtCore import Qt, QRectF, QObject, QThread, Signal, Slot, QTimer, QStandardPaths, QUrl
...
from ...utils.camera import VideoRecorder, VideoRecorderWorker, get_transformed_frame
...
class CameraControlWidget(QWidget):
    ...
    send_frame_to_worker = Signal(QImage, QRectF, QRectF, QTransform)
    ...
    def capture_button_handler(self, capture_data=None):
        ...
        # NOTE: Screenshots still run on main thread using the local helper
        ...
        if was_camera_off: self.restore_camera_state()
    ...
    def video_record_start(self, directory=None, step_description=None, step_id=None, show_dialog=True):
        ...
        if self.recorder.start(self.recording_file_path, resolution, current_fmt.maxFrameRate()):
            ...
            # Connect Camera Feed
            self.video_item.videoSink().videoFrameChanged.connect(self.video_record_frame_handler)
    ...
    def _show_media_capture_dialog(self, name: str, save_path: str):
        ...
        if self.show_media_capture_dialog:
            success(None, formatted_message, title=f"{name.title()} Captured")
