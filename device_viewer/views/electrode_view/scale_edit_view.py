...
ApplyScaleButton = Action(name="Apply Scale", action="set_electrode_area_scale")
...
class ScaleEditViewController(Controller):
    # --- User input ---
    real_electrode_scale = Float(1.0)  # User sets this value
    electrode_interaction_service = Instance(ElectrodeInteractionControllerService)

    # --- Calculated Property ---
    scaling_factor = Property(Float, observe=['real_electrode_scale', 'model.area_scaled'])

    def _get_scaling_factor(self):
        """Calculates the scaling factor to be displayed."""
        if self.model.area_scaled > 0:
            return self.real_electrode_scale / self.model.area_scaled
        return 0.0
