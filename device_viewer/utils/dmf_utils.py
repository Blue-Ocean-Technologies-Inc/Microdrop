...
class SvgUtil(HasTraits):
    ...
    connections_modified = Bool(
        False,
        desc="whether the connections differ from the file's Connections "
        "layer (auto generated or hand edited) and need writing on save",
    )
    zone_records = List(
    ...
    def add_connection(self, from_id, to_id):
        """Connect two electrodes; a self-connection or an existing one is
        left alone."""
        if from_id == to_id or to_id in self.neighbours.get(from_id, []):
            return

        neighbours = {key: list(ids) for key, ids in self.neighbours.items()}
        neighbours.setdefault(from_id, []).append(to_id)
        neighbours.setdefault(to_id, []).append(from_id)

        # Reassigned, not mutated: the connections rebuild observes the
        # whole dict.
        self.neighbours = neighbours
        self.connections_modified = True

    def remove_connections(self, electrode_id_pairs):
        """Disconnect each (electrode id, electrode id) pair, whichever
        direction it is given in."""
        removed = {frozenset(pair) for pair in electrode_id_pairs}
        ...
        self.neighbours = {
            key: [
                ...
                if frozenset((key, neighbour)) not in removed
            ]
            for key, ids in self.neighbours.items()
        }
        self.connections_modified = True
    ...
    def neighbours_to_points(self, event):
        # Built locally and assigned once, so ``connections`` observers
        # see the finished dict rather than the empty one.
        connections = {}

        for k, v in self.neighbours.items():
            for n in v:
                if (n, k) not in connections and (k, n) not in connections:
                    ...
                    connections[(k, n)] = (coord_k, coord_n)
    ...
    def save_to_file(self, file, electrode_ids_channels_map: dict[str, int]):
        ...
        ### Add connections if auto generated or hand edited ###
        if self.connections_modified:
            logger.info("Writing modified connections to save file")
            ...
            # the file's Connections layer now matches again
