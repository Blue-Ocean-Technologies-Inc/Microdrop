...
def _make_capturing_executor_factory():
    """Returns (factory, captured_dict).

    The factory produces a minimal fake executor whose .start(**kw)
    records the kwargs so tests can assert on extra_scratch et al.
    The fake satisfies all the wiring hooks in ProtocolTreePane.__init__
    (qsignals, stop, pause_event, resume, pause).
    """
    from unittest.mock import MagicMock
    captured = {}
...
def test_pane_omits_extra_scratch_when_no_provider(qapp):
    ...
    must not pass an electrode_areas extra_scratch — the executor's
    default behaviour (protocol_metadata only) applies."""
    import pluggable_protocol_tree.views.protocol_tree_pane as ptp
    from pluggable_protocol_tree.builtins.name_column import make_name_column
