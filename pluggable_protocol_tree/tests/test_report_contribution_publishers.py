...
class TestContributionPublishers:
    def test_publishers_bind_their_validators(self):
        # validator_class is a Traits ``Type`` trait, not a plain class
        # attribute — its default only resolves through an instance.
        assert (
            ProtocolLoggingMetadataContributionPublisher().validator_class
            is ProtocolLoggingMetadataContributionMessage
        )
        assert (
            ProtocolLoggingDataContributionPublisher().validator_class
    ...
    def test_consts_singletons_bound_to_their_topics(self):
        ...
        assert (
            protocol_logging_metadata_contribution_publisher.topic
            == PROTOCOL_LOGGING_METADATA_CONTRIBUTION
        )
        assert (
            protocol_logging_data_contribution_publisher.topic
            == PROTOCOL_LOGGING_DATA_CONTRIBUTION
        )
