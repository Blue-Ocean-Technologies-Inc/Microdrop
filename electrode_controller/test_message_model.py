...
def test_duplicate_values_are_deduplicated():
    ...
    payload = {"channels": [1, 2, 2, 5, 5]}
    context = {"max_channels": 10}

    model = ElectrodeChannelsRequest.model_validate(payload, context=context)

    assert model.channels == {1, 2, 5}
...
def test_missing_key_rejected():
    """Test that failing to provide the 'channels' key raises an error."""
...
def test_json_dump():
    ...
    message = ElectrodeChannelsRequest(channels=data["channels"])
    dumped_json = json.loads(message.model_dump_json())

    assert type(dumped_json["channels"]) == list
...
def test_model_instantiation_failure():
    ...
    data = {"channels": [1, 2, 5, "2"]}
    with pytest.raises(ValidationError) as exc_info:
        ElectrodeChannelsRequest(channels=data["channels"])
