...
from dropbot_preferences_ui.consts import VoltageFrequencyRangePreferences
...
def _make_manual_control_model():
    """Build the ManualControlModel class with ranges from the UI preferences.
    ...
    Group(
        Group(
            Item(
                ...
                label="Voltage (V)",
                resizable=True,
                # enabled_when='realtime_mode': changes are simply not
                # published instead of disabling the editor.
            ),
            Item(
                name="frequency",
                label="Frequency (Hz)",
...
class ManualControlControl(Controller):
    ...
    # This is likely because update_editor is only called by 'external' trait
    # changes, and the new thread spawned by the decorator appears as such
