...
def test_plugin_contributes_three_columns():
    """Tasks 3-5 added Video, Record, and Capture — the default factory
    yields three entries. Capture is a compound (#396) whose model has no
    col_id, so identity is read via the IColumn/ICompoundColumn-common
    `id` trait rather than `model.col_id`."""
    from video_protocol_controls.plugin import VideoProtocolControlsPlugin

    p = VideoProtocolControlsPlugin()
    cols = p._contributed_protocol_columns_default()
    assert len(cols) == 3
    assert [c.id for c in cols] == ["video", "record", "capture"]
