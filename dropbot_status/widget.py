...
class DropBotStatusWidget(BaseDramatiqControllableDropBotQWidget):
    ...
    def _on_capacitance_updated_triggered(self, body):
        if self.realtime_mode: # Only update the capacitance and voltage readings if we are in realtime mode
            capacitance = json.loads(body).get('capacitance', '-') # Both are strings that have units generated by pint
            voltage = json.loads(body).get('voltage', '-')
            voltage_value = float(voltage.split(" ")[0])

            pressure = None
            force = None

            if self.filler_capacitance_over_area and self.liquid_capacitance_over_area:
                pressure = self.liquid_capacitance_over_area - self.filler_capacitance_over_area
                force = ForceCalculationService.calculate_force_for_step(
                    voltage_value,
                    pressure
            )

            logger.critical(f"Capacitance: {capacitance}, Voltage: {voltage}, Pressure: {pressure}, Force: {force}")

            self.status_label.update_pressure_reading(f"{pressure:.4f} pF/mm^2" if pressure is not None else "-")
            self.status_label.update_force_reading(f"{force:.4f} mN/m" if force is not None else "-")
            self.status_label.update_capacitance_reading(f"{capacitance}" if capacitance is not None else "-")
            self.status_label.update_voltage_reading(f"{voltage}" if voltage is not None else "-")
