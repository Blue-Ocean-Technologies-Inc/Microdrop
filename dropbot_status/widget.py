...
class DropBotStatusWidget(BaseDramatiqControllableDropBotQWidget):
    def __init__(self):
        ...
        self.liquid_capacitance_over_area = 0
    ...
    def _on_capacitance_updated_triggered(self, body):
        if self.realtime_mode: # Only update the capacitance and voltage readings if we are in realtime mode
            capacitance = json.loads(body).get('capacitance', '-') # Both raw values have units generated by pint
            ...
            self.status_label.update_pressure_reading(f"{pressure:.4f}" if pressure is not None else "-")
            self.status_label.update_force_reading(f"{force:.4f}" if force is not None else "-")
